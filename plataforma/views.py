...
def home(request):
    ...
    if preco_minimo or preco_maximo or cidade or tipo:
        ...
        if not tipo:
            tipo = ['A', 'C']
        if cidade == '0':
            imoveis = Imovei.objects.filter(valor__gte=preco_minimo).filter(valor__lte=preco_maximo)\
            .filter(tipo_imovel__in=tipo).filter(disponibilidade = 'D')
        else:
            imoveis = Imovei.objects.filter(valor__gte=preco_minimo).filter(valor__lte=preco_maximo)\
        .filter(tipo_imovel__in=tipo).filter(cidade=cidade).filter(disponibilidade = 'D')

    else: imoveis = Imovei.objects.all().filter(disponibilidade = 'D')
